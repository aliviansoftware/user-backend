...
func createUserHandler_should_pass_User_object_to_UserService_CreateUser(t *testing.T) {
	...
	var result *pkg.User
	us.CreateUserFn = func(u *pkg.User) error {
		result = u
		return nil
	}
...
func profileHandler_should_return_User_from_context(t *testing.T) {
	...
	test_mux := NewUserRouter(&us, mux.NewRouter(), &ah)
	var result string
	us.GetUserByUsernameFn = func(username string) (error, pkg.User) {
		result = username
		return nil, pkg.User{}
	}

	testUsername := "test_username"
	testUser := pkg.User{Username: testUsername}
...
func profileHandler_should_return_StatusNotFound_if_no_user_found(t *testing.T) {
	...
	us.GetUserByUsernameFn = func(username string) (error, pkg.User) {
		result = username
		return errors.New("user service error"), pkg.User{}
	}
	testUsername := "test_username"
	testUser := pkg.User{Username: testUsername}
...
func loginHandler_should_provide_new_auth_cookie_if_userService_returns_a_user(t *testing.T) {
	...
	test_mux := NewUserRouter(&us, mux.NewRouter(), &ah)
	var result string
	us.LoginFn = func(credentials pkg.Credentials) (error, pkg.User) {
